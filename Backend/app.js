...
app.get('/',(req, res)=>{
    res.json({"Hello": "To the SwipTory Server"});
})
...
app.post('/login', async (req, res)=>{
    ...
    if(authentication) {
        try {
        const jwtToken = await jwt.sign({username, password}, process.env.JWT_SECRET_KEY, { expiresIn: 1800})
        res.json({
            authentication: true,
            login: "successful",
            token: jwtToken,
...
app.post('/story', isAuthenticated, async (req, res) =>{
    ...
    if(found[0]==undefined) {
        try {
            await swipToryStory.create({storyID, heading, description, imageURL, category})
            return res.json({"Success":"Story Successfully Uploaded", storyID:storyID, iteration: 0})
        }
        catch(e) {
    ...
    if(found) {
        try {
            ...
            if(iteration>=6)
            return res.json({"Error":"Not allowed to upload more than 6 slides in a story"})
            await swipToryStory.create({storyID, heading, description, imageURL, category, iteration: iteration+1})
            return res.json({"Success":"Story Successfully Uploaded", storyID: storyID, iteration: iteration+1})
        }
        catch(e) {
            return res.json({"Error": e})
        }
...
app.post('/like', isAuthenticated, async (req, res) =>  {
    try {
        ...
        console.log(check)
        if(check==null || check==undefined) return res.json({error: "This story doesn't exist in the database"})
        const found = await swipToryStory.findOneAndUpdate({storyID, iteration: 0}, {$inc: {likes: 1}}, {
            new: true
        })
        return res.json({found})
        }
...
app.post('/bookmark', isAuthenticated, async (req, res)=>{
    try {
        ...
        return res.json({error: "Bookmark already exists. Try delete request."})
        const found = await swipToryUser.findOneAndUpdate({username: user}, {
        ...
        res.json({error: "You are missing either user or storyID",
    "ERR": e})    
    }
})
...
app.delete('/bookmark', isAuthenticated, async (req, res) => {
    try {
        ...
        res.json({Error: "User or StoryID is missing",
                err: e})
    }
})
